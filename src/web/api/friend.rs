...
pub async fn send_friend_request(req: HttpRequest, data: Data<AppState>, path: Path<String>) -> WebResult {
    ...
    let to_user_id = crate::database::user::resolve_friend_code(&data.pool, &to_friend_code.simple().to_string()).await.map_err(to_web_error)?;
...
pub async fn accept_friend_request(req: HttpRequest, data: Data<AppState>, path: Path<String>) -> WebResult {
    ...
    let from_user_id = crate::database::user::resolve_friend_code(&data.pool, &from_user_code.simple().to_string()).await.map_err(to_web_error)?;
...
pub async fn get_settings(req: HttpRequest, data: Data<AppState>, path: Path<UserId>) -> WebResult {
    ...
    let friend_id = path.into_inner();
    let settings = crate::database::friend::get_friend_settings(&data.pool, token.user_id, friend_id).await.map_err(to_web_error)?;
    ok(settings)
}
